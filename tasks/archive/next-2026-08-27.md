# NEXT.md archive — split 2026-08-27

Verbatim. Nothing below was summarised, reworded, shortened or dropped; this
file plus `tasks/NEXT.md` reconstruct the pre-split file byte for byte.

Filed by the date of the SPLIT, not of the entries, following
`next-2026-08-25.md` and `next-2026-08-18.md`. These are the 2026-08-24,
2026-08-23, 2026-08-22, 2026-08-21 and 2026-08-20 entries that were still in
`NEXT.md` when it reached **259,407 bytes -- 98.9% of the 262,144-byte ceiling
at which the Read tool refuses a file outright**.

Split at 98.9% rather than at the 90% the file's own rule names, because the
entry that crossed the line was the one being written. The rule is unchanged and
the next split should happen earlier: what breaks first is not the test, it is
the instruction at the top of `NEXT.md` -- a session that cannot read the whole
file reads the head and silently believes it has the state.
