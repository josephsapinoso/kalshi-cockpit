# Archive - `tasks/NEXT.md` session entries, split 2026-09-08

**Verbatim.** These entries were moved out of `tasks/NEXT.md` unchanged,
byte for byte, when that file stood at 205,381 bytes - 78.3% of the
262,144-byte ceiling at which the Read tool refuses a file outright.
Nothing was summarised, reworded or dropped.

**Taken under the rule rather than at it, and deeper than it required.**
The trigger is ~90%; this was cut at 78.3%, before the session's own entry
was written, because clearing to well under the line buys one more split
rather than several. The cut falls on a **date boundary** - everything
2026-09-06 and earlier moved - so no single day is split across two files.

The moved block hashes `md5 = 823e72d9643035f974168e24d26b440c`.
