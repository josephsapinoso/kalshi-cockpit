# Archive - `tasks/NEXT.md` session entries, split 2026-09-11

**Verbatim.** These entries were moved out of `tasks/NEXT.md` unchanged,
byte for byte, when that file stood at 200701 bytes - 76.6% of the
262,144-byte ceiling at which the Read tool refuses a file outright.
Nothing was summarised, reworded or dropped.

**Taken under the rule rather than at it, and before the session's own
entry was written.** The trigger is ~90%; this was cut at 76.6%, because
clearing to well under the line buys one more split rather than several,
and because the entry about to be written was a long one -- the size that
matters is the size *after*. The cut falls on a **date boundary**:
everything 2026-09-09 and earlier moved, so no single day is split across
two files.

Seven entries -- five from 2026-09-09, two from 2026-09-08. Verified by md5 -- the bytes below
this header hash identically to the bytes removed from `tasks/NEXT.md`:
`e993d4b77d01bb4e823d1ed53e49302a`. Index lines moved in the same edit.
