...
**The `/wayfinder` skill is not installed in the current plugin version (checked 2026-09-08)**; nothing can invoke it. The issue format below still stands and the map (#3) is read and written with `gh` directly.
...
- **Open a ticket for Joe — CLAUDE.md workflow step 7.** A measurement that
  raises a decision only Joe can make opens the child **in the same session**;
  a line in `tasks/NEXT.md` decays into the instrument that produced it. Three
  commands, and the third is the one people forget (an issue that is not linked
  as a sub-issue is invisible to the frontier query):
  ...
  ticket+letter in one line); the evidence, with `file:line` or the instrument
