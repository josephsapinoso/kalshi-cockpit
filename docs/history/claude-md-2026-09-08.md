# CLAUDE.md as it stood on 2026-09-08, before the cut

This is the spine file **verbatim** as of commit 25ad5fb, preserved when it was
cut from 44KB to the rules and facts (ADR 0116). The corrected facts stayed in
`CLAUDE.md`; this file is the trail of how each one got corrected — every
"this paragraph used to say", every struck-through defect, every dated audit.
Open it when a sentence in the spine sounds like it has a history and you need
to know what that history was. Nothing below is maintained; the live rule is
whatever `CLAUDE.md` says now.
