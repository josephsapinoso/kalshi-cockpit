# DRAFT — The venue's own fill numbers are kept on the permanent row

**No ordinal.** The number is taken in the merge commit, after `git fetch`, as
the last thing before the push (`docs/adr/README.md`).
