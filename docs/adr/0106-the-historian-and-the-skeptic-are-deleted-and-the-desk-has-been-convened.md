# 0106 — The Historian and the Skeptic are deleted, and the desk has been convened

**Status:** Accepted. Lane C, 2026-09-05. The ordinal was taken at the merge
boundary after `git fetch` (`docs/adr/README.md`): 0105 was the highest on
`origin/main` and no other unmerged lane claimed 0106. The eight files that
cited this document by its `DRAFT-` path were renumbered in the same commit
(`backend/agents/review.py`, `backend/agents/base.py`, `backend/playbook.py`,
`tests/test_has_callers.py`, `tests/test_agent_wiring.py`,
`tests/test_agent_budget.py`, `tests/test_agents.py`, `docs/adr/0071-*.md`).
**Date:** 2026-09-05.
