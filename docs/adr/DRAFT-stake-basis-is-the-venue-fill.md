# DRAFT — The stake is read at the venue's own fill price, on the read

Written by lane B (`lane/stake-basis-venue-fill`) with **no ordinal**; the
number is taken at the merge commit after `git fetch`, per
`docs/adr/README.md`. **No schema change, no `SCHEMA_VERSION` claimed.**
