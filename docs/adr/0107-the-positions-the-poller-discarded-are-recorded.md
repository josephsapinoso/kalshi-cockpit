# 0107 — The positions the poller discarded are recorded, and "staked" is exposure at cost

**Status:** Accepted. Lane A1, 2026-09-05, the partner's rank-1 item, step 1
of 2 — step 2 (lane A2) is the screen. The ordinal was taken at the merge
boundary after `git fetch` (`docs/adr/README.md`): 0105 was the highest on
`origin/main`, Lane C took 0106 in the same batch, and `SCHEMA_VERSION` was
re-read as 32 on `origin/main` before v33 was accepted from this lane.
**The integrator's edit that section 9 named is made in the merge commit**,
so the hand-bet route's read feeds the money figure and the "one population
and not two" sentence in its docstring is wholly true rather than half.
**Date:** 2026-09-05.
...
because `routes.py` is lane B's file this session; §9 names the lines.
**It was made at the merge**, after lane B's split landed, so the paragraph
below describes an interval that lasted one merge rather than a shipped
state — see the correction at the end of §6. What
...
- ~~**That the hand-bet route keeps its rows.** It does not, as of this ADR
  ...
  > **Closed at the merge, 2026-09-05.** The §9 edit is made: the route passes
  > `position_rows` to `_stamp_positions_read`, which calls
  > `store_positions_snapshot` inside the same transaction, so its stamp is
  > marked and its rows are kept. The docstring's "one population and not two"
  > is now wholly true and says so. Four guards in
  > `tests/test_manual_orders.py::TestTheLivePositionsReadIsStamped` cover it,
  > two of them mutation-verified red (removing the snapshot call; letting a
  > failed read mirror).
  >
  > **One thing the integrator found that this lane could not.** The guard is
  ...
  > blocks it and a mutation deleting `ok and` stayed green through the API.
  > It is pinned by calling `_stamp_positions_read` directly instead, which is
  > the only level the branch is reachable from. A guard whose only mutation
  > is green is decoration, and this one is now not.
...
- **That the screen renders the figure.** That is lane A2. This lane touched
  neither `frontend/` nor `backend/api/routes.py` — the `routes.py` edit
  described above is the integrator's, made at the merge, and `frontend/` is
  still untouched.
...
- ~~**The integrator's edit to `routes.py`, deferred from this lane.**~~
  **Done at the merge, 2026-09-05** — see the correction in §6. The
  instruction is left below because its line numbers are the record of where
  the seam was, and because the second half of the paragraph (what the marker
  buys in the interval) is still the reason the marker exists. **Note the
  line numbers are pre-split**: lane B moved `_stamp_positions_read` from
  6138 to ~3735 in the same batch, so the edit was located by symbol. In
